...
function Render.draw(world, camera, texture, gui_texture, icons_texture, inventory_texture, target, player, selected_block, day_time, hotbar, inventory, mode, dead, inventory_open)
    ...
    if icons_texture then
        local key = "crosshair:" .. tostring(icons_texture:getWidth()) .. ":" .. tostring(icons_texture:getHeight())
        local q = Render.icon_quads[key]
        if not q then
            q = love.graphics.newQuad(0, 0, 16, 16, icons_texture:getWidth(), icons_texture:getHeight())
            Render.icon_quads[key] = q
        end
        love.graphics.setColor(1, 1, 1, 0.85)
        love.graphics.draw(icons_texture, q, cx - 16, cy - 16, 0, 2, 2)
    else
    ...
    local slot_scale = 2
    local using_1122_gui = gui_texture and gui_texture:getWidth() == 256 and gui_texture:getHeight() == 256
    local using_1122_icons = icons_texture and icons_texture:getWidth() == 256 and icons_texture:getHeight() == 256
    local slot_source_size = using_1122_gui and 20 or 22
    ...
    local slot_quad
    local selected_slot_quad
    local selected_slot_offset = -2
    if using_1122_gui then
        slot_quad = getGuiQuad("slot_1122", 0, 0, 20, 20)
        selected_slot_quad = getGuiQuad("slot_selected_1122", 0, 20, 24, 24)
    elseif custom_gui then
        slot_quad = getGuiQuad("slot_custom", 0, 16, 22, 22)
        selected_slot_quad = getGuiQuad("slot_selected_custom", 22, 16, 22, 22)
        selected_slot_offset = 0
    else
        slot_quad = getGuiQuad("slot_original", 0, 0, 22, 22)
        selected_slot_quad = getGuiQuad("slot_selected_original", 0, 22, 24, 22)
    end
    
    -- Новые ico1122.png сердца лежат 16x16-тайлами: empty=16,0 full=32,0 half=48,0.
    local heart_scale = 2
    local heart_full, heart_half, heart_empty
    if using_1122_icons then
        heart_full = getIconQuad("heart_full_1122", 32, 0, 16, 16)
        heart_half = getIconQuad("heart_half_1122", 48, 0, 16, 16)
        heart_empty = getIconQuad("heart_empty_1122", 16, 0, 16, 16)
        heart_scale = 18 / 16
    else
        ...
        heart_full = icons_texture and getIconQuad("heart_full_original", 52, 0, 9, 9) or getGuiQuad("heart_full_custom", 0, 0, 9, 9)
        heart_half = icons_texture and getIconQuad("heart_half_original", 61, 0, 9, 9) or getGuiQuad("heart_half_custom", 9, 0, 9, 9)
        heart_empty = icons_texture and getIconQuad("heart_empty_original", 16, 0, 9, 9) or getGuiQuad("heart_empty_custom", 18, 0, 9, 9)
    end
    ...
    for i = 1, hearts do
        ...
        if q and heart_texture then
            ...
            love.graphics.draw(heart_texture, q, hearts_x + (i - 1) * 18, hearts_y, 0, heart_scale, heart_scale)
        else
    ...
    for i, block_id in ipairs(hotbar) do
        ...
        if not quad then
            ...
            quad = love.graphics.newQuad(u * tex_w, v * tex_h, Block.TILE_SIZE, Block.TILE_SIZE, tex_w, tex_h)
            Render.icon_quads[quad_key] = quad
        end
        
        love.graphics.setColor(1, 1, 1, count > 0 and 1 or 0.35)
        local item_pad = using_1122_gui and 4 or 6
        local scale_factor = 32 / Block.TILE_SIZE -- Иконка 32x32 на экране
        love.graphics.draw(texture, quad, x + item_pad, hotbar_y + item_pad, 0, scale_factor, scale_factor)
    ...
    if inventory_open then
        local panel_src_w, panel_src_h = 176, 166
        local inv_scale = 2
        local panel_w, panel_h = panel_src_w * inv_scale, panel_src_h * inv_scale
        ...
        if inventory_texture then
            local key = "inventory_panel:" .. tostring(inventory_texture:getWidth()) .. ":" .. tostring(inventory_texture:getHeight())
            local panel_quad = Render.gui_quads[key]
            if not panel_quad then
                panel_quad = love.graphics.newQuad(0, 0, panel_src_w, panel_src_h, inventory_texture:getWidth(), inventory_texture:getHeight())
                Render.gui_quads[key] = panel_quad
            end
            love.graphics.setColor(1, 1, 1, 1)
            love.graphics.draw(inventory_texture, panel_quad, panel_x, panel_y, 0, inv_scale, inv_scale)
        else
        ...
        for row = 0, 2 do
            for col = 0, 8 do
                table.insert(slot_positions, {x = 8 + col * 18, y = 84 + row * 18})
            end
        end
        for col = 0, 8 do
            table.insert(slot_positions, {x = 8 + col * 18, y = 142})
        end
        
        local item_ids = {}
        for id, count in pairs(inventory) do
            if id ~= 0 and (count or 0) > 0 then table.insert(item_ids, id) end
        end
        table.sort(item_ids)
        
        for idx, block_id in ipairs(item_ids) do
            local slot = slot_positions[idx]
            if not slot then break end
            local quad_key = "inv:" .. tostring(block_id) .. ":" .. tostring(tex_w) .. ":" .. tostring(tex_h)
            local quad = Render.icon_quads[quad_key]
            if not quad then
                local u, v = Block.getUV(block_id, "top")
            ...
            local draw_x = panel_x + (slot.x + 1) * inv_scale
            local draw_y = panel_y + (slot.y + 1) * inv_scale
            love.graphics.setColor(1, 1, 1, 1)
            love.graphics.draw(texture, quad, draw_x, draw_y, 0, inv_scale, inv_scale)
            
            local count_text = tostring(inventory[block_id] or 0)
            local count_w = (Render.font and Render.font.getWidth) and Render.font:getWidth(count_text) or (#count_text * 10)
            love.graphics.setColor(0, 0, 0, 0.9)
            love.graphics.print(count_text, draw_x + 30 - count_w + 2, draw_y + 16 + 2)
            love.graphics.setColor(1, 1, 1, 1)
            love.graphics.print(count_text, draw_x + 30 - count_w, draw_y + 16)
        end
        
        local title = "Inventory (E)"
        local title_w = (Render.font and Render.font.getWidth) and Render.font:getWidth(title) or (#title * 10)
        drawMinecraftText(title, math.floor((screen_w - title_w) / 2), panel_y - 28)
    end
