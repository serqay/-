...
local function addTorchMesh(vertices, chunk, wx, y, wz, id)
    ...
    local uv_w, uv_h = Block.getUVSize()
    local eps_x, eps_y = 0.005 * uv_w, 0.005 * uv_h
    local u1, v1 = u + eps_x, v + eps_y
    local u2, v2 = u + uv_w - eps_x, v + uv_h - eps_y
                ...
                if id and id ~= 0 then
                    ...
                    if Block.getModel(id) == "torch" then
                        ...
                        if is_transparent then
                            ...
                            local uv_w, uv_h = Block.getUVSize()
                            local eps_x, eps_y = 0.005 * uv_w, 0.005 * uv_h
                            local u1, v1_uv = u + eps_x, v + eps_y
                            local u2, v2_uv = u + uv_w - eps_x, v + uv_h - eps_y
